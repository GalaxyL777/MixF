...
class QUVs_likelihood_bilby(bilby.Likelihood):
    def __init__(self, nu, Q, U, V, Q_err, U_err, V_err, Model):
        '''
        A simple likelihood to fit the propagation model with QUV.
        '''
        super().__init__(parameters={'log10n0L': None, 'log10T': None,
                                     'log10B': None, 'thetaB': None,
                                     'chip':None,
                                     'beta0':None, 'chi0':None,
                                     'RMb': None, 'RMf': None})
        # n0L, T, B, theta_B, chi, I0, Q0, U0, V0, RM
        self.nu = nu
        self.Q = Q
    ...
    def update(self, log10n0L, log10T, log10B, thetaB, chip, beta0, chi0, RMb, RMf):
        self.parameters['log10n0L'] = log10n0L
        self.parameters['log10T']  = log10T
        self.parameters['log10B']  = log10B
        self.parameters['thetaB']  = thetaB
        self.parameters['chip']  = chip
        self.parameters['beta0'] , self.parameters['chi0'] = beta0, chi0
        self.parameters['RMb'] = RMb
        self.parameters['RMf'] = RMf
    
    def log_likelihood(self):
        # get the parameters
        log10n0L = self.parameters['log10n0L']
        log10T = self.parameters['log10T']
        log10B = self.parameters['log10B']
        thetaB = self.parameters['thetaB']
        chip = self.parameters['chip']
        beta0, chi0 = self.parameters['beta0'], self.parameters['chi0']
        RMb, RMf = self.parameters['RMb'], self.parameters['RMf']
        ...
        self.Model.parameters_update(n0L, T, B, thetaB, chip, I0, Q0, U0, V0, RMb, RMf)
        Qm, Um, Vm = self.Model.Stokes_calculate(self.nu)
